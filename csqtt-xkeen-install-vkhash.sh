...
# 3. Запрос VK Access Token для автоматического обновления хешей
echo ""
echo "VK Access Token нужен для ежедневного создания нового звонка."
echo "Токен будет сохранён в ${VK_TOKEN_FILE} с правами 600."

while true; do
    if [ -t 0 ]; then
        printf "Введите VK Access Token (обязательно): "
        stty -echo 2>/dev/null || true
        read -r VK_ACCESS_TOKEN
        stty echo 2>/dev/null || true
        echo ""
    else
        printf "Введите VK Access Token (обязательно): " > /dev/tty
...
# 6. Создание скрипта ежедневного обновления VK-хеша
