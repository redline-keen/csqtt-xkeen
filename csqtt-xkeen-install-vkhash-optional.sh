...
VK_TOKEN_FILE="${INSTALL_DIR}/vk_access_token"
HASH_UPDATE_SCRIPT="/opt/bin/csqtt-hash-update.sh"
HASH_UPDATE_LOG="${INSTALL_DIR}/hash-update.log"

echo "=== Установка и настройка csqtt-client ==="

# Для ежедневного обновления VK-хешей нужен curl.
if ! command -v curl >/dev/null 2>&1; then
    echo "Установка curl для обновления VK-хешей..."
...
echo "Оставьте поле пустым, чтобы использовать родные хеши csqtt."

if [ -t 0 ]; then
    printf "VK Access Token (Enter — пропустить): "
    stty -echo 2>/dev/null || true
    read -r VK_ACCESS_TOKEN
    stty echo 2>/dev/null || true
    echo ""
else
    printf "VK Access Token (Enter — пропустить): " > /dev/tty
    stty -echo < /dev/tty 2>/dev/null || true
    read -r VK_ACCESS_TOKEN < /dev/tty
    stty echo < /dev/tty 2>/dev/null || true
    echo "" > /dev/tty
fi

if [ -n "$VK_ACCESS_TOKEN" ]; then
    echo "$VK_ACCESS_TOKEN" > "${VK_TOKEN_FILE}"
    chmod 600 "${VK_TOKEN_FILE}"
    VK_HASH_UPDATE_ENABLED=1
    echo "VK Token сохранён. Будет включено ежедневное обновление одного случайного hash."
else
    rm -f "${VK_TOKEN_FILE}"
    VK_HASH_UPDATE_ENABLED=0
    echo "VK Token не указан. Будут использоваться родные хеши csqtt."
fi
unset VK_ACCESS_TOKEN
...
CONF_FILE="/opt/etc/csqtt/csqtt.conf"
TOKEN_FILE="/opt/etc/csqtt/vk_access_token"
LOG_FILE="/opt/etc/csqtt/hash-update.log"
LOCK_FILE="/opt/etc/csqtt/hash-update.lock"
TMP_FILE="/opt/etc/csqtt/csqtt.conf.hash.tmp"
VK_API="https://api.vk.com/method/calls.start"
VK_API_VERSION="5.131"

log() {
    echo "$(date '+%Y-%m-%d %H:%M:%S') [HASH] $*" >> "$LOG_FILE"
}

if [ -f "$LOCK_FILE" ]; then
    OLD_PID=$(cat "$LOCK_FILE" 2>/dev/null)
    if [ -n "$OLD_PID" ] && kill -0 "$OLD_PID" 2>/dev/null; then
        log "Уже запущено (PID $OLD_PID), выходим."
        exit 0
    fi
    rm -f "$LOCK_FILE"
fi
...
[ -f "$CONF_FILE" ] || { log "ОШИБКА: нет $CONF_FILE"; exit 1; }
[ -s "$TOKEN_FILE" ] || { log "ОШИБКА: нет VK Access Token"; exit 1; }
command -v curl >/dev/null 2>&1 || { log "ОШИБКА: curl не найден"; exit 1; }

VK_ACCESS_TOKEN=$(cat "$TOKEN_FILE")
[ -n "$VK_ACCESS_TOKEN" ] || { log "ОШИБКА: пустой VK Access Token"; exit 1; }

RESPONSE=$(curl -ksS --max-time 30 --retry 2 \
    --data-urlencode "access_token=${VK_ACCESS_TOKEN}" \
    --data "v=${VK_API_VERSION}" \
    --data "allow_anonymous=1" \
    "$VK_API" 2>/dev/null) || {
    log "ОШИБКА: VK API недоступен."
    exit 1
}

JOIN_LINK=$(echo "$RESPONSE" | sed -n 's/.*"join_link":"\([^"]*\)".*/\1/p' | head -n 1)

if [ -z "$JOIN_LINK" ]; then
    ERROR_MSG=$(echo "$RESPONSE" | sed -n 's/.*"error_msg":"\([^"]*\)".*/\1/p' | head -n 1)
    [ -n "$ERROR_MSG" ] || ERROR_MSG="не удалось получить join_link"
    log "ОШИБКА VK: $ERROR_MSG"
    exit 1
fi

NEW_HASH=$(echo "$JOIN_LINK" | sed 's/[?#].*$//' | sed 's#.*/##')
[ -n "$NEW_HASH" ] || { log "ОШИБКА: не удалось извлечь hash из join_link."; exit 1; }

VK_LINE=$(grep '^VK=' "$CONF_FILE" 2>/dev/null | head -n 1)
[ -n "$VK_LINE" ] || { log "ОШИБКА: строка VK= не найдена."; exit 1; }
...
case "$RAND_BYTE" in
    ''|*[!0-9]*) SLOT=$(( ( $(date +%s) + $$ ) % HASH_COUNT + 1 )) ;;
    *) SLOT=$(( RAND_BYTE % HASH_COUNT + 1 )) ;;
esac

NEW_VK=""
INDEX=1
for HASH in "$@"; do
    if [ "$INDEX" -eq "$SLOT" ]; then
        HASH="$NEW_HASH"
    fi

    if [ -z "$HASH" ]; then
        log "ОШИБКА: пустой hash на позиции $INDEX."
        exit 1
    fi
...
NEW_COUNT=$(grep '^VK=' "$TMP_FILE" | sed "s/^VK=['\"]//;s/['\"]$//" | tr ',' '\n' | wc -l | tr -d ' ')
[ "$NEW_COUNT" = "$HASH_COUNT" ] || {
    log "ОШИБКА: контроль количества хешей не пройден ($HASH_COUNT -> $NEW_COUNT)."
    exit 1
}
...
log "OK: новый hash установлен в позицию $SLOT из $HASH_COUNT."
exit 0
EOF
...
HASH_CRON_JOB="0 4 * * * /opt/bin/csqtt-hash-update.sh >/dev/null 2>&1"
...
if [ "$VK_HASH_UPDATE_ENABLED" = "1" ]; then
    if ! grep -Fq "/opt/bin/csqtt-hash-update.sh" /opt/var/spool/cron/crontabs/root 2>/dev/null; then
        echo "$HASH_CRON_JOB" >> /opt/var/spool/cron/crontabs/root
    fi
else
    sed -i '/\/opt\/bin\/csqtt-hash-update\.sh/d' /opt/var/spool/cron/crontabs/root 2>/dev/null || true
fi
...
if [ -f "/opt/var/spool/cron/crontabs/root" ]; then
    ...
    sed -i '/\/opt\/bin\/csqtt-hash-update\.sh/d' /opt/var/spool/cron/crontabs/root 2>/dev/null || true
    rm -f "/opt/etc/csqtt/vk_access_token" "/opt/etc/csqtt/hash-update.log" "/opt/etc/csqtt/hash-update.lock"
    rm -f "/opt/bin/csqtt-hash-update.sh"
    if [ -x "/opt/etc/init.d/S10cron" ]; then
        /opt/etc/init.d/S10cron restart >/dev/null 2>&1 || true
    fi
fi
