...
verify_bin() { # verify_bin ПУТЬ — ELF + разрядность/endianness совпадают
    ...
    magic=$(head -c 6 "$1" | od -An -tx1 | tr -d ' \n')
    [ "$magic" = "7f454c460201" ] || [ "$magic" = "7f454c460101" ] \
        || die "$1 не является 64/32-битным ELF-бинарником (получен: $magic)"
    case "$ARCH_KEY" in
        aarch64) [ "${magic#7f454c4602}" != "$magic" ] || die "ожидался 64-битный ELF" ;;
        mipsel)  [ "${magic#7f454c460101}" != "$magic" ] || die "ожидался 32-битный LE ELF" ;;
    esac
    chmod +x "$1"
}
...
# smoke: бинарник должен ругнуться про -peer
smoke=$("$BIN_PATH" 2>&1 | head -n 1)
printf '%s\n' "$smoke" | grep -q "peer" \
    || die "бинарник не отвечает ожидаемым образом: $smoke"
log "Бинарник установлен и отвечает: $smoke"
