...
# 1.5. VK access token (необязательно) — генерация хешей прямо на роутере
# Пустой ввод = работать только с хешами из csqtt:// ссылки (как раньше).
VK_TOKEN=""
VK_HASH_COUNT="2"
if [ -t 0 ]; then
    printf "Введите VK access token (Enter = только хеши из ссылки): "
    read -r VK_TOKEN
else
    printf "Введите VK access token (Enter = только хеши из ссылки): " > /dev/tty
    read -r VK_TOKEN < /dev/tty
fi

if [ -n "$VK_TOKEN" ]; then
    if [ -t 0 ]; then
        printf "Сколько hash генерировать по токену (2..6, по умолчанию 2): "
        read -r VK_HASH_COUNT
    else
        printf "Сколько hash генерировать по токену (2..6, по умолчанию 2): " > /dev/tty
        read -r VK_HASH_COUNT < /dev/tty
    fi
    case "$VK_HASH_COUNT" in
        ''|*[!0-9]*) VK_HASH_COUNT="2" ;;
        *) [ "$VK_HASH_COUNT" -lt 2 ] && VK_HASH_COUNT="2"
           [ "$VK_HASH_COUNT" -gt 6 ] && VK_HASH_COUNT="6" ;;
    esac
fi

# 2. Запрос потоков (от 9 до 162)
# Подобрано под железо Keenetic:
#   MT7621 (mipsel 880 MHz, 256 МБ) — 9 воркеров по умолчанию
#   MT7981 (aarch64 1.3 GHz, 512 МБ) — 18 воркеров по умолчанию
case "$ARCH" in
    mipsel) DEFAULT_THREADS=9 ;;
    arm64)  DEFAULT_THREADS=18 ;;
    armv7)  DEFAULT_THREADS=9  ;;
    *)      DEFAULT_THREADS=18 ;;
esac

while true; do
    if [ -t 0 ]; then
        printf "Введите количество потоков [-n] (по умолчанию %s, диапазон от 9 до 162): " "$DEFAULT_THREADS"
        read -r THREADS_INPUT
    else
        printf "Введите количество потоков [-n] (по умолчанию %s, диапазон от 9 до 162): " "$DEFAULT_THREADS" > /dev/tty
...
printf "PEER='%s'\nPASSWORD='%s'\nVK='%s'\nVK_TOKEN='%s'\nVK_HASH_COUNT='%s'\nTUN='%s'\nN='%s'\n" \
    "$PEER" "$PASSWORD" "$VK" "$VK_TOKEN" "$VK_HASH_COUNT" "$TUN" "$N" > "${CONF_FILE}"
chmod 600 "${CONF_FILE}"

# 4. Установка бинарника — локально или из сети
echo "[2/6] Установка бинарного файла (${ARCH})..."
# Сначала ищем локальный бинарь рядом со скриптом (офлайн-режим)
SCRIPT_DIR=$(cd "$(dirname "$0")" 2>/dev/null && pwd)
LOCAL_BIN=""
for candidate in \
    "${SCRIPT_DIR}/${BIN_NAME}-${ARCH}" \
    "${SCRIPT_DIR}/csqtt-client-${ARCH}" \
    "/opt/tmp/${BIN_NAME}-${ARCH}" \
    "/opt/tmp/csqtt-client-${ARCH}"; do
    if [ -f "$candidate" ]; then
        LOCAL_BIN="$candidate"
        break
    fi
...
if [ -n "$LOCAL_BIN" ]; then
    echo "  Использую локальный бинарь: ${LOCAL_BIN}"
    cp "${LOCAL_BIN}" "${TARGET_PATH}"
elif [ -n "${BIN_URL}" ]; then
    echo "  Локальный бинарь не найден, скачиваю с ${BIN_URL}"
    ...
    echo "  ОШИБКА: не найден ни локальный бинарь, ни URL для скачивания." >&2
    exit 1
fi
chmod +x "${TARGET_PATH}"
...
start() {
    ...
    (
        # VK_TOKEN непустой — клиент сам генерирует пул hash по токену
        # (--vk-token/--vk-hash-count); хеши из ссылки остаются fallback-ом.
        if [ -n "$VK_TOKEN" ]; then
            "$PROG" --peer "$PEER" --password "$PASSWORD" --vk "$VK" \
                --vk-token "$VK_TOKEN" --vk-hash-count "$VK_HASH_COUNT" \
                --tun "$TUN" -n "$N" 2>&1 | while IFS= read -r line; do
                echo "$line" >> "$LOGFILE"
                count=$((count + 1))
